...
modelo_censo22 <- readLines("modelos/censo_2022.txt", encoding = "UTF-8")
meta_censo22 <- read_csv("metadados/censo22.csv")

#Categoriza se a variante é Relativa ou Absoluta
meta_censo22 <- meta_censo22 %>% 
  filter(!is.na(indicator_id)) %>% 
  mutate(categoria = str_sub(source_table_id, -3, -1)) %>% 
  mutate(categoria = as.factor(categoria))

meta_wFormulas_censo22 <- meta_censo22 %>% 
  left_join(formula_censo22, by = c("indicator_id" = "indicator_id",
...
for (indicator_id_ in unique(meta_wFormulas_censo22$indicator_id)) {
  tmp <- modelo_censo22
  Encoding(tmp) <- "UTF-8"
  
  indicator_label <- filter(meta_wFormulas_censo22, indicator_id == indicator_id_)$indicator_label[1]
  
  tmp[1] <- sub("\\{\\{m_title\\}\\}", indicator_label, tmp[1])
  
  ## Cria seção de variantes absolutas
  vars_abs <- meta_wFormulas_censo22 %>% 
  ...
  vars_rel <- meta_wFormulas_censo22 %>%
  ...
  ##Cria arquivo
  writeLines(tmp, paste0("censo/2022/", indicator_id_,".md"),
             useBytes = T)
